...
(function (K) {
  ...
  P.home = () => {
    ...
    return `<div class="hm-top"><div class="card hi one"><p><b>New?</b> Follow the ${link('quests', '', 'Main quest')} and read ${link('systems', 'runs_saving', 'Runs and saving')} before your first -save. Unknown word? See ${link('systems', 'terms', 'Terms')}.</p></div>
    ...
    <div class="hm-grid">
      ...
      ${card('tier', 'Tier list', 'S to C per mode and difficulty (N1-3, N4-6, N7-9)')}
      ${card('planner', 'Run planner', 'Load your save, get your best next run (Legacy gear or Points)')}
      ...
      ${card('legacy', 'Legacy', `${n(L.length)} gear lines kept between games, ${n(steps)} upgrade steps`)}
      ${card('quests', 'Quests', `${n(((W.qguide || {}).steps || (W.quests || {}).main || []).length)} main quest steps, ${n(((W.quests || {}).side || []).length)} side quests`)}
      ${card('systems', 'Game systems', `Difficulty, Points, enhancing and more (${n((W.sys || []).length)} pages) + ${n((W.commands || []).length)} chat commands`)}
      ${card('calc', 'Calculators', 'Points per run, runs to a title, enhancing cost in Boss Souls')}
    </div>`;
  };
})(window.AP);
